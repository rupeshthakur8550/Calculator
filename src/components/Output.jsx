...
const Output = () => {
    ...
    return (
        <div className='header bg-gray-700 px-5 py-2 h-[40%] w-full flex flex-col justify-end text-right overflow-hidden font-semibold text-white'>
            <div ref={historyRef} className='header_history overflow-y-auto scrollbar-hide my-2 text-gray-500 text-xs sm:text-lg'>
            ...
            <div className='header_expression'>
                <p ref={expressionRef} className='text-xl sm:text-4xl overflow-x-auto scrollbar-hide'>
                    {expression}
                </p>
                {
                    expression.length === 0 && <p className='text-xl sm:text-4xl'>{result}</p>
                }
            </div>
        </div>
    );
